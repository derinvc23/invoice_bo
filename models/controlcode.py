...
class ControlCode(object):
    ...
    def generate(self, number, x_total, **kwargs):
        ...
        if not self.validate_amount(x_total):
            raise InvalidParam('Invalid amount %s' % x_total)
        ...
        ## step 1
        ##
        verhoeff5 = self._step1_verhoeff_5digits(number, nit, date_invoice, x_total)
        ...
        for s1,s2 in zip((auth, number, nit, date_invoice, x_total), strings):
    ...
    #
    def _step1_verhoeff_5digits(self, number, nit, date_invoice, x_total):
        # dummy verhoeff numbers
        data = (number, nit, date_invoice, x_total)
...
#            raise InvalidParam('Invalid auth %s' % auth)
    ...
    def set_amount(self, x_total):
        if self.validate_amount(x_total):
            self._amount = x_total
        else:
            raise InvalidParam('Invalid amount %s' % x_total)
        return self
    ...
    def validate_amount(self, x_total):
        return isinstance(x_total, (int,float,long)) and x_total > 0
...
#VARIABLES NEEDED FOR THE PROCESS TO WORK: Next we detail each of them
    ...
    control_code = cc.generate(data['number'], data['x_total'])
