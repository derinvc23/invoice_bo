...
class account_journal(models.Model):
    ...
    direccion_sucursal = fields.Text('Direccion Casa Matriz',
                                     help='Es la direccion, telefonos, ciudad de la Casa Matriz que aparece en las facturas')
    direccion_sucursal_2 = fields.Text('Direccion de la Sucursal',
                                       help='Es la direccion, telefonos, ciudad de la sucursal que aparece en las facturas')
    ...
    mensaje_factura = fields.Char('Mensaje en Factura',
                                  help='Mensaje que se imprime en todas las facturas en la parte inferior')
...
class account_invoice(models.Model):
    ...
    autorizacion = fields.Char(related='journal_id.autorizacion_dosificacion', string="Autorizacion", store=True,
                               readonly=True)
    llave = fields.Char(related='journal_id.llave_dosificacion', string="Llave de la dosificacion", store=True,
                        readonly=True)
    fecha = fields.Date(related='journal_id.fecha_limite', string="Limite de emision", store=True, readonly=True)
    direccion = fields.Text(related='journal_id.direccion_sucursal', string="Direccion de la Casa Matriz", store=True,
                            readonly=True)
    direccion_2 = fields.Text(related='journal_id.direccion_sucursal_2', string="Direccion de la sucursal", store=True,
                              readonly=True)
    actividad = fields.Char(related='journal_id.actividad_dosificacion', string="Actividad del contribuyente",
                            store=True, readonly=True)
    leyenda = fields.Text(related='journal_id.leyenda_dosificacion', string="Leyenda de la factura", store=True,
                          readonly=True)
    leyenda2 = fields.Text(related='journal_id.leyenda_secundaria', string="Leyenda de la dosificacion", store=True,
                           readonly=True)
    nit_empresa = fields.Char(related='journal_id.nit_contribuyente', string="Nit contribuyente", store=True,
                              readonly=True)
    sucursal = fields.Char(related='journal_id.nombre_sucursal', string="Casa Matriz", store=True, readonly=True)
    sucursal_2 = fields.Char(related='journal_id.nombre_sucursal_2', string="Nombre de la sucursal", store=True,
                             readonly=True)
    ...
    estado_factura = fields.Char(related='journal_id.estado_factura', string="Estado Factura", store=True,
                                 readonly=True)
    ...
    def invoice_validate(self):
        ...
        for inv in self:
            if inv.type in ('out_invoice'):
                ...
                control_code = cc.generate(number, inv.x_total)
                if not inv_date or not number or not inv.x_total:
